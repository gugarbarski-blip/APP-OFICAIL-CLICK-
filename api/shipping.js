...
function tablePrice(table, weightG, zone) {
  const row = table.find(function(r) { return weightG <= r.maxWeight; }) || table[table.length - 1];
...
async function calcFrenet(cepDestino, numBoxes, weightPerBoxKg) {
  ...
  const tid = setTimeout(function() { controller.abort(); }, 7000);
  ...
  const valid = services.filter(function(s) {
  ...
  try {
    ...
    if (process.env.FRENET_TOKEN) {
      try {
        ...
        combined.push.apply(combined, frenetResults);
      } catch (err) {
    ...
    const temPAC   = combined.some(function(r) { return /\bpac\b/i.test(r.label) || r.service.includes('04510'); });
    const temSEDEX = combined.some(function(r) { return /\bsedex\b/i.test(r.label) || r.service.includes('04014'); });

    if (!temPAC || !temSEDEX) {
      const ufKey = ufParam.toUpperCase().trim();
      let uf = (ufParam && UF_ZONE[ufKey]) ? ufKey : null;
    ...
    combined.sort(function(a, b) { return a.price - b.price; });
    return res.status(200).json({ options: combined, numBoxes: numBoxes, realWeightPerBoxKg: realWeightPerBoxKg });
